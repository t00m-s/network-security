...
int main(int argc, char* argv[]) {
  ...
  if (raw_socket == -1) {
    perror("Usage: sudo ./traceroute_server <interface_name>\n");
    exit(EXIT_FAILURE);
  }
...
void filter_icmp(char *buffer) {
  ...
  // I trust it.
  struct ipheader *ip_1_header =
      (struct ipheader *)(buffer + sizeof(struct ether_header));
  if (ip_1_header->iph_protocol != 1)
    return;
  ...
  icmp_saddr.s_addr = ip_1_header->iph_sourceip.s_addr;
  ...
  size_t ip_2_header_size = (((struct ipheader *)(payload))->iph_ihl * 4);
  total_skip_size += ip_2_header_size;
  total_skip_size += sizeof(struct udpheader);
